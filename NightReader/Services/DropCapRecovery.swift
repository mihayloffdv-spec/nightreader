...
//
// Восстановление пропущенных первых символов (drop caps) в тексте PDF.
// Три стратегии: CMap-декодирование шрифтов → OCR (Vision) → словарный подбор.
//
// ┌──────────────────────────────────────────────────────┐
// │              DropCapRecovery                          │
// │                                                      │
...
// │  extractCMapCharacters(cgPage) → [CMapChar]          │
// │  diagnoseDropCaps(document) → report                 │
// └──────────────────────────────────────────────────────┘

enum DropCapRecovery {
    // MARK: - Drop cap recovery
    ...
    static func recoverDropCaps(
        ...
        for candidate in candidates {
            ...
            let isLikelyFragment = TextExtractor.isFragment(searchPrefix)
    ...
    private static func guessMissingPrefix(fragment: String) -> String? {
        ...
        let prefixRules: [(prefix: String, insert: String)] = [
            ...
            // Added from Codex analysis of real PDF failures:
            ("ебрант", "С"), ("ебастьян", "С"), ("ервер", "С"), ("екунд", "С"),
            ("ока ", "П"), ("ока,", "П"), ("оказ", "П"), ("оказат", "П"),
            ("ресловут", "П"), ("реслов", "П"),
            ("онечно", "К"), ("онтент", "К"), ("онкурен", "К"), ("онверс", "К"),
            ("ри этом", "П"), ("ри чём", "П"), ("ри чем", "П"),
            ("тандартн", "С"), ("тавк", "С"), ("тратег", "С"), ("тоимост", "С"),
            ("езусловно", "Б"), ("юджет", "Б"),
            ("еличин", "В"), ("ещи", "В"), ("едь", "В"),
            ("сходн", "И"), ("нтернет", "И"), ("зменит", "И"), ("менно", "И"),
            ("аркетинг", "М"), ("аркетолог", "М"),
            ("аголовок", "З"), ("аявк", "З"), ("аработ", "З"),
            ("олич", "К"), ("олонк", "К"), ("амфор", "К"),
        ]
        ...
        for pageIndex in 0..<document.pageCount {
            ...
            for (pattern, _) in patterns {
                ...
                for match in matches {
                    ...
                    guard TextExtractor.isFragment(word) else { continue }
                    let confidence = "HIGH"
